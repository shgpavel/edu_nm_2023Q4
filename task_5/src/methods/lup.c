...
void lup(matrix *a, vector *b, vector *solution) {
	...
	for (size_t i = a->rows; i > 0; ) {
		...
		for (size_t j = a->rows; j > i + 1; ) {
			--j;
			double tmp;
			tmp = unwrap_double(vector_get(solution, i)) -
        (unwrap_double(matrix_get(a, i, j)) * unwrap_double(vector_get(solution, j)));
			vector_change(solution, i, (void *)&tmp);
		}
		double tmp;
		tmp = unwrap_double(vector_get(solution, i)) / unwrap_double(matrix_get(a, i, i));
		vector_change(solution, i, (void *)&tmp);
	}
